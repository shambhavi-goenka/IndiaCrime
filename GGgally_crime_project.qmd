...
```{r}
IPC_combined_India_districtlevel <- st_cast(IPC_combined_India_districtlevel, "POLYGON")
SLL_combined_India_districtlevel <- st_cast(SLL_combined_India_districtlevel, "POLYGON")
```
...
There are 1056 regions in India. The most connected area unit has 15 neighbours. And there are 57 regions with just 1 neighbour.

```{r}
longitude_IPC <- map_dbl(IPC_combined_India_districtlevel$geometry, ~st_centroid(.x)[[1]])
```

```{r}
latitude_IPC <- map_dbl(IPC_combined_India_districtlevel$geometry, ~st_centroid(.x)[[2]])
```

```{r}
coords <- cbind(longitude_IPC, latitude_IPC)
```
...
```{r}
#wm62_lw <- nb2listw(wm_d62, style = 'B', set.ZeroPolicyOption(TRUE))
#summary(wm62_lw)
```

```{r}
knn_IPC <- knn2nb(knearneigh(coords, k=8))
knn_IPC
```

```{r}
rswm_IPC <- nb2listw(knn_IPC, style = 'B')
summary(rswm_IPC)
```
...
```{r}
MI_corr <- sp.correlogram(knn_IPC, 
